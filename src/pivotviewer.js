...
(function ($) {
    var _views = [],
        ...
        printTile, //_tile that is used when exporting data #AR
		printFilters, //specific filter used when exporting data #AR
		printSortFacet = ""; //title of category used when exporting data #AR
    ...
    var methods = {
        ...
        init: function (options) {
            ...
            .always(function (defaultOptions) {
                ...
                $('.pv-wrapper').append("<div id=\"pv-file-selection\" class=\"pv-modal-dialog\"><div><a href=\"#pv-modal-dialog-close\" title=\"Close\" class=\"pv-modal-dialog-close\">X</a><p><select id=\"pv-server-file\" class=\"pv-server-file\"><option>Select a file</option></select><p><input id=\"pv-load-file\" class=\"pv-load-file\" type=file accept=\".csv\"><p>Setup New Data</div></div>");

                $pv_server_file = $('#pv-server-file');
                $.getJSON("../project_list.json", function (data) {
                    $.each(data, function (key, value) {
                        $pv_server_file.append('<option value=\"' + value + '\">' + key + '</option>');
                    });
                });
                ...
                $pv_server_file.on('change', function (e) {
                    options.Loader = new PivotViewer.Models.Loaders.CXMLLoader($pv_server_file.val());
                    InitCollectionLoader(options, defaultOptions);
                    window.open("#pv-modal-dialog-close", "_self");
                });
                
                $('.pv-load-file').on('change', function (e) {                   
                    var fileInput = document.getElementById("pv-load-file");
                    options.Loader = new PivotViewer.Models.Loaders.LocalCSVLoader(fileInput.files[0]);
                    window.open("#pv-modal-dialog-close", "_self");
                });
    ...
    InitCollectionLoader = function (options, defaultOptions) {
        InitPreloader();
    ...
    CreateViews = function () {
        ...
        _views.push(new PivotViewer.Views.ExportView()); //EXPORTVIEW INSTANCE #AR
    ...
    SelectView = function (viewNumber, init) {
        ...
        // Makes sure graphview is only called once #AR {
        if (viewNumber == 5) {
            // var b = 0;
            // 			var i = b++;
            // 			console.log("called " + i +" times");
            // 			i++;
            _views[5].Filter(printTile, printFilters, printSortFacet, _stringFacets);
            return;
        }
        // }#AR
        ...
        if (_currentView == 1 && (viewNumber == 2 || viewNumber == 3 || viewNumber == 4 || viewNumber == 5)) { // #AR
